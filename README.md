...
- **The summary carries the aggregates an adviser asks for**, not just a list of days: totals and a per-day average for feeds and nappies, average sleep, the typical gap between feeds and between nappies with the shortest and longest either side of it, the longest overnight stretch, and the longest single sleep. Gaps are measured inside the period you chose rather than borrowed from the forecast, which answers a different question and would contradict the days listed above it. Averages divide by the days that have entries, so three days of log inside a fortnight is three days. It also states plainly that feeds are logged as moments rather than spans, since an assistant asked how long a feed lasted will otherwise assume the number was simply left out.
